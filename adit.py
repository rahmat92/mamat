...
aditmadzs = LineClient(authToken='EvE7V0cXKAGBZ6GyjC31.arRdWLw1YQBhxDcwv15meq.JSgNI9PV5Y4gWWcN6G4ewVozp6HljJ6NBZ59B83MRAI=')
...
#ki = LineClient()
#ki = LineClient(authToken='Evi84JfW0nZ1LkiSNqL9.LCJKe1bkHTRkBUJOTAC52q.CX8gJ4kmfV4uhzdlQLNiXdpOxk5i7bcstWwRIHsc9uk=')
...
call = aditmadzs
creator = ["u011ac3c255b5caa6c75dd096575e4c11"]
owner = ["u011ac3c255b5caa6c75dd096575e4c11"]
admin = ["u011ac3c255b5caa6c75dd096575e4c11"]
staff = ["u011ac3c255b5caa6c75dd096575e4c11"]
...
def siderMembers(to, mid):
    try:
        arrData = ""
        textx = "PELAKU SISITIPI「{}」\n\nHaii Kk ".format(str(len(mid)))
        arr = []
        no = 1
        num = 2
        for i in mid:
...
def welcomeMembers(to, mid):
    try:
        arrData = ""
        textx = "ASSALAMU ALAIKUM「{}」\n\nSalken Kk  ".format(str(len(mid)))
        arr = []
        no = 1
        num = 2
        for i in mid:
...
def leaveMembers(to, mid):
    try:
        arrData = ""
        textx = "MEMBER BAPER「{}」\n\nGood Byee Kk  ".format(str(len(mid)))
        arr = []
        no = 1
        num = 2
        for i in mid:
            ...
            arrData = {'S':slen, 'E':elen, 'M':i}
            arr.append(arrData)
            textx += mention+wait["leave"]+"\nMEMBER BAPER DI GROUP "+str(ginfo.name)
            if no < len(mid):
                no += 1
                textx += "%i " % (num)
                num=(num+1)
            ...
            if mid in op.param3:
                if wait["autoJoin"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        aditmadzs.acceptGroupInvitation(op.param1)
                        ginfo = aditmadzs.getGroup(op.param1)
                        aditmadzs.sendMessage(op.param1,"Haii,,,semua nyaa salken yaa aim anak baru jangan di bully yaa^^")
                    else:
                        aditmadzs.acceptGroupInvitation(op.param1)
                        ginfo = aditmadzs.getGroup(op.param1)
                        aditmadzs.sendMessage(op.param1,"Haii,,,semua nyaa salken yaa aim anak baru jangan di bully yaa^^")
                    ...
                    if msg_id in msg_dict:
                        if msg_dict[msg_id]["from"]:
                           if msg_dict[msg_id]["text"] == 'Gambarnya dibawah':
                                ...
                                arifAR = aditmadzs.getContact(msg_dict[msg_id]["from"])
                                ret_ =  "「 Pesan Dihapus 」\n"
                                ret_ += "• Pelaku : {}".format(str(arifAR.displayName))
                 ...
                 if wait["detectMention"] == True:
                   ...
                   for mention in mentionees:
                        if mention ['M'] in Bots:
                           ...
                           aditmadzs.sendMessage(msg.to, None, contentMetadata={"STKID":"9805240","STKPKGID":"5923","STKVER":"1"}, contentType=7)
                           break
               if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["Mentiongift"] == True:
                   ...
                   for mention in mentionees:
                        if mention ['M'] in Bots:
                           ...
                           aditmadzs.sendMessage(msg.to, "Kenapa Kk suka ngetag aim minta di gift yaa!?\nCek di PM, udah aku gift tuh...")
                           aditmadzs.sendMessage(msg.to, None, contentMetadata={"STKID":"16844053","STKPKGID":"8670","STKVER":"3"}, contentType=7)
               ...
               if msg.contentType == 0:
                    ...
                    if text is None:
                        ...
                        if cmd == "unsend off":
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               sendMention(msg.to, sender, "「 Informasi SelfBOT 」\n")
                               aditmadzs.sendMessage(msg.to, None, contentMetadata={'mid': mid}, contentType=13)

                        elif cmd == "aim" or text.lower() == 'me':
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               msg.contentType = 13
...
#===========BOT UPDATE============#
                          ...
                          if wait["selfbot"] == True:
                               ...
                               nama1, nama2, nama3, nama4, jml = [], [], [], [], len(nama)
                               if jml <= 20:
                                   mentionMembers(msg.to, nama)
                               if jml > 20 and jml < 40:
                                   for i in range (0, 20):
                                       nama1 += [nama[i]]
                                   mentionMembers(msg.to, nama1)
                                   for j in range (20, len(nama)-1):
                                       nama2 += [nama[j]]
                                   mentionMembers(msg.to, nama2)
                               ...
                               if jml > 100 and jml < 120:
                                   ...
                                   for l in range (60, 80):
                                       nama4 += [nama[l]]
                                   mentionMembers(msg.to, nama4)
                                   for m in range (100, len(nama)-1):
                                       nama5 += [nama[m]]
                                   mentionMembers(msg.to, nama5)
                               if jml > 120 and jml < 140:
                                   ...
                                   for m in range (80, 100):
                                       nama5 += [nama[m]]
                                   mentionMembers(msg.to, nama5)
                                   for n in range (120, len(nama)-1):
                                       nama6 += [nama[n]]
                                   mentionMembers(msg.to, nama6)
                        ...
                        elif cmd == "tikung" or cmd == "sp":
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               start = time.time()
                               aditmadzs.sendMessage(msg.to, "Progres kecepatan nikung...")
                               elapsed_time = time.time() - start
                               aditmadzs.sendMessage(msg.to, "{} detik kecepatan nikung".format(str(elapsed_time)))
                          ...
                          if wait["selfbot"] == True:
                           if msg._from in admin:
                              try:
                                  tz = pytz.timezone("Asia/Jakarta")
                                  timeNow = datetime.now(tz=tz)
                                  aditmadzs.sendMessage(msg.to, "Cek jones diaktifkan\n\nTanggal : "+ datetime.strftime(timeNow,'%Y-%m-%d')+"\nJam [ "+ datetime.strftime(timeNow,'%H:%M:%S')+" ]")
                          ...
                          if wait["selfbot"] == True:
                           if msg._from in admin:
                              if msg.to in cctv['point']:
                                  ...
                                  aditmadzs.sendMessage(msg.to, "Cek jones dinonaktifkan\n\nTanggal : "+ datetime.strftime(timeNow,'%Y-%m-%d')+"\nJam [ "+ datetime.strftime(timeNow,'%H:%M:%S')+" ]")
                              else:
